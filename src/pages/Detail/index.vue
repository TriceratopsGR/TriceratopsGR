...
export default {
  ...
  methods: {
    ...
    async addShopcar() {
      ...
      // 返回的是一个 Promise
      try {
        //调用 vuex 中的这个addUpdateShopCart函数
        let res = await this.$store.dispatch("addUpdateShopCart", {
          skuId: this.$route.params.skuid,
          skuNum: this.skuNum,
        });
        //成功 路由 路由跳转 传递参数=>产品信息带给下一级的路由组件
        this.$router.push({
          name: "addCartSuccess",
          query: { skuNum: this.skuNum },
        });
        // 存储
        // 一些简单的数据skuNum 通过query 形式给路由组件传递过去
        // 产品信息的数据【比较复杂：skuInfo】,通过会话存储（不持久化，会话结束 数据消失）;
        // 本地、会话存储一般存储的字符串  不能存对象
        // 存
          ...
          .cartWrap {
            ...
            .add {
              ...
              a {
                ...
                &:link:hover {
                  color: rgb(195, 231, 35) !important;
